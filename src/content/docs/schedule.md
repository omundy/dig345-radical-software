---
title: Schedule
description: Get started building your docs site with Starlight.
---


<style>
.calloutBox {
	border: 1px solid #999;
	padding: 1rem;
}
</style>


## Week 1

#### 1-1 Intro; create a Jamstack website



1. Before class
	1. 📚 Reading(s)
1. In class 
	1. Install NVM and Node.js, Grunt.js tutorials


#### 1-2 Jamstack; Static Site Generators (SSGs)



1. Before class
	1. 📚 Reading(s)
	1. 🎬 Heydon Pickering [Why is everything binary?](https://www.youtube.com/watch?v=LbjxvvKEtyE)
1. In class 
	1. [Astro Starlight](tutorials/astro-starlight/) tutorial

<div class="calloutBox">

##### HW1: Create a Jamstack website to post your work this semester

1. Follow the [Astro Starlight](tutorials/astro-starlight/) tutorial to build your website
1. Experiment with content and organization. Starlight uses the [Diátaxis](https://diataxis.fr/reference/) approach to technical documentation authoring 
1. Create a schedule section and add a new markdown file for week 1.
1. Write a short reflection in this page using this structure:
	- What did you learn this week? Explain and link to sources.
	- ...
1. Publish the page and link to it directly on the appropriate Moodle forum

</div>










## Week 2


#### 2-1 Node Express


#### 2-1 Vercel


Might use this? 
https://docs.google.com/document/d/1caPqg3nRV4VFWZVMy-ujeGHbPA11CmUReCon8_d8MVQ/edit?tab=t.0



<div class="calloutBox">

#### HW2 - Create a Node/Express server 

1. 

</div>






## Week 3


#### 3-1


#### 3-1 

#### Examples

- https://patatap.com/


<div class="calloutBox">

#### HW3 - Create an interactive soundscape

1. Follow the [Sound & Music tutorial](tutorials/sound-music) to learn Tone.js and Strudel.
1. Continue learning Strudel on their [workshop](https://strudel.cc/workshop/first-sounds/)
1. Explore the [device motion](https://strudel.cc/learn/devicemotion/) and [input devices](https://strudel.cc/learn/input-devices/)
1. Design and code an interactive experience that makes use of studel. Consider using shapes, color, and other visuals in addition to sounds. It should have ...
1. Post and "about" page on your Jamstack site with a screenshot, video, and 100-150 word statement about the concept and technology

</div>







## Week 4 - Reactivity


#### 4-1

1. Before class
   1. [React in 100 Seconds](https://www.youtube.com/watch?v=Tn6-PIqc4UM)
   1. 🎬 Heydon Pickering [What Is React.JS?](https://www.youtube.com/watch?v=BO71b3l22mM)
1. In class 
   1. [Three.js installation](https://threejs.org/manual/#en/installation) 
   
#### 4-1 


<div class="calloutBox">

#### HW4 - Create a reactive website

1. 

</div>


	




## Week 5


#### 5-1 Three.js 

1. Before class
	1. 📚 Read [0:Welcome](https://discoverthreejs.com/book/introduction/)
	1. 🎬 Heydon Pickering [What's The Deal With Large Language Models?](https://www.youtube.com/watch?v=rchEbQNMepw)
1. In class 
	1. [Three.js installation](https://threejs.org/manual/#en/installation) 

#### 5-1 Three.js 

1. Before class
	1. 📚 Read [1:Getting Started](https://discoverthreejs.com/book/first-steps/)
1. In class 
	1. TBD tutorial


<div class="calloutBox">

### hw4 - Three.js 

1. 

</div>

















## Week 6





## Week 7




## Week 8-15

Final project










